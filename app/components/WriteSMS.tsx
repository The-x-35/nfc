...
import { writeSMSToNFC, isNFCSupported } from '../lib/nfc';

export default function WriteSMS() {
  ...
  const handleWrite = async () => {
    ...
    try {
      await writeSMSToNFC(phone, message || undefined);
      setSuccess(true);
      setPhone('');
      setMessage('');
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <h1 className="text-3xl font-bold text-white mb-2">Write SMS</h1>
        <p className="text-slate-400">Write an SMS link to your NFC tag</p>
      </div>
      ...
      {success && (
        <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-4">
          <div className="flex items-center gap-3">
            ...
            <p className="text-green-400">Successfully wrote SMS to NFC tag!</p>
          </div>
        </div>
      )}
